...
function createWorkflowTestCmd() {
    return __awaiter(this, void 0, void 0, function* () {
        let name = yield vscode.window.showInputBox({
            prompt: "Enter the workflow test name",
            placeHolder: "e.g., send_email",
        });
        if (!name) {
            vscode.window.showErrorMessage("Workflow test name is required");
            return;
        }
        name = name.replace(/([^a-zA-Z0-9/]+)/g, "_");
        const workflowTestContent = `workflow_test "${name}" {
  stack {
    expect.to_be_true (1 == 1)
  }
}`;
        try {
            const uri = vscode.Uri.file((0, path_1.join)(config_1.ROOT_PATH, config_1.config.paths.workflowTests, `${(0, lodash_1.snakeCase)(name)}.xs`));
            ...
            // create the file
            yield vscode.workspace.fs.writeFile(uri, Buffer.from(workflowTestContent));
            (0, openXanoscriptEditor_1.openXanoscriptUri)(uri);
            vscode.window.showInformationMessage(`Workflow test ${name} created successfully`);
