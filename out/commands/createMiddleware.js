...
function createMiddlewareCmd() {
    return __awaiter(this, void 0, void 0, function* () {
        let name = yield vscode.window.showInputBox({
            prompt: "Enter the middleware name",
            placeHolder: "e.g., auth_middleware",
        });
        if (!name) {
            vscode.window.showErrorMessage("Middleware name is required");
            return;
        }
        name = name.replace(/([^a-zA-Z0-9/]+)/g, "_");
        const middlewareContent = `middleware "${name}" {
  ...
  output {
    json response
        ...
        try {
            const uri = vscode.Uri.file((0, path_1.join)(config_1.ROOT_PATH, config_1.config.paths.middlewares, `${(0, lodash_1.snakeCase)(name)}.xs`));
            ...
            // create the file
            yield vscode.workspace.fs.writeFile(uri, Buffer.from(middlewareContent));
            (0, openXanoscriptEditor_1.openXanoscriptUri)(uri);
            vscode.window.showInformationMessage(`Middleware ${name} created successfully`);
            ...
            vscode.window.showErrorMessage("Failed to create middleware");
            return;
        }
    });
}
